                ...
                @foreach ($disfraces as $disfraz)
                    <div class="col">
                        <div class="card h-100 shadow-sm">
                            @if($disfraz->imagen)
                                <img src="{{ asset('storage/' . $disfraz->imagen) }}" class="card-img-top"
                                    style="height: 200px; object-fit: cover;" alt="Imagen de {{ $disfraz->nombre }}">
                            @else
                                <img src="https://via.placeholder.com/300x200?text={{ urlencode($disfraz->nombre) }}"
                                    class="card-img-top" style="height: 200px; object-fit: cover;" alt="Sin imagen">
                            @endif

                            <form method="POST" action="{{ route('cliente.carrito.agregar') }}">
                                @csrf
                                <input type="hidden" name="disfraz_id" value="{{ $disfraz->id }}">
                                <input type="number" name="cantidad" min="1" value="1" class="form-control mb-2"
                            ...
                            <div class="card-footer bg-transparent border-top-0">
                                <a href="{{ route('cliente.alquiler.form', $disfraz->id) }}"
                                    class="btn btn-primary w-100">Alquilar</a>
                            </div>
                        </div>
                    </div>
                @endforeach
