...
<div class="container mt-5">
    ...
    <div class="card p-4 mb-4 shadow-sm">
        <h5>Registrar Nuevo Alquiler</h5>
        <form action="{{ route('admin.alquileres.store') }}" method="POST">
            ...
            <div class="row">
                ...
                <div class="col-12">
                    <button class="btn btn-success">Registrar Alquiler</button>
                </div>
            </div>
        </form>
    </div>
    ...
    @else
        <div class="table-responsive shadow-sm">
            <table class="table table-bordered table-hover align-middle">
                ...
                <tbody>
                    @php $edit = request('edit'); @endphp
                    @foreach($alquileres as $a)
                        @php
                            $hoy = \Carbon\Carbon::now()->startOfDay();
                            $fin = \Carbon\Carbon::parse($a->fecha_fin)->startOfDay()->addDay(); // se considera día completo
                            $retraso = $hoy->gt($fin) && $a->estado != 'finalizada';
                            $diasRetraso = $retraso ? $fin->diffInDays($hoy) : 0;
                            $sancion = $diasRetraso * 10;
                            $totalFinal = $a->total + $sancion;
                        @endphp
                        ...
                        @if($edit == $a->id)
                            <form action="{{ route('admin.alquileres.update', $a->id) }}" method="POST">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select name="estado" class="form-select form-select-sm">
                                            @foreach(['reservada','activa','retrasada','finalizada','cancelada'] as $estado)
                                                <option value="{{ $estado }}" {{ $a->estado === $estado ? 'selected' : '' }}>
                                                    {{ ucfirst($estado) }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <td>S/. {{ number_format($totalFinal, 2) }}</td>
                                    <td>
                        ...
                        @else
                            <tr>
                                ...
                                <td>
                                    ...
                                    <form action="{{ route('admin.alquileres.destroy', $a->id) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Eliminar este alquiler?')">
                                        @csrf @method('DELETE')
