...
<div class="container mt-5">
    ...
    <div class="mb-3">
        <button class="btn btn-primary" onclick="toggleFormulario()">+ Nuevo Alquiler</button>
    </div>

    {{-- Formulario para nuevo alquiler --}}
    <div id="formulario-alquiler" class="card p-4 mb-4 shadow d-none">
        <form method="POST" action="{{ route('admin.alquileres.store') }}">
            @csrf
            <div class="row mb-3">
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    <select name="disfraz_id" class="form-select" required>
                        ...
                        @foreach($disfraces as $disfraz)
                            <option value="{{ $disfraz->id }}" data-precio="{{ $disfraz->precio }}">{{ $disfraz->nombre }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="total" step="0.01" class="form-control" id="total" required>
            </div>

            <div class="d-flex justify-content-end gap-2">
                <button type="submit" class="btn btn-success">Guardar Alquiler</button>
                <button type="button" onclick="toggleFormulario()" class="btn btn-secondary">Cancelar</button>
            </div>
        </form>
    </div>
                ...
                <tbody>
                    @foreach($alquileres as $alquiler)
                        @php
                            ...
                            $diasRetraso = $hoy->gt($fechaFin) && $alquiler->estado !== 'finalizada' ? $fechaFin->diffInDays($hoy) : 0;
                        ...
                        @if($editId == $alquiler->id)
                            <form action="{{ route('admin.alquileres.update', $alquiler->id) }}" method="POST">
                                ...
                                <tr>
                                    ...
                                    <td>{{ $diasRetraso > 0 ? 'S/ ' . number_format($montoSancion, 2) : '-' }}</td>
