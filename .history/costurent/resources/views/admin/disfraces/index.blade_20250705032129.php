...
<div class="container mt-5">
    ...
    @foreach ($categorias as $categoria)
        @php
            $cards = $disfraces->where('categoria_id', $categoria->id);
        @endphp
        @if ($cards->count())
            <h3 class="mt-5">{{ $categoria->nombre }}</h3>
            <div id="carouselCategoria{{ $categoria->id }}" class="carousel slide mb-5" data-bs-ride="carousel">
                <div class="carousel-inner">
                    @foreach ($cards->chunk(3) as $chunkIndex => $chunk)
                        <div class="carousel-item {{ $chunkIndex == 0 ? 'active' : '' }}">
                            <div class="row row-cols-1 row-cols-md-3 g-4">
                                @foreach ($chunk as $disfraz)
                                    <div class="col">
                                        <div class="card h-100 shadow">
                                            @if ($disfraz->imagen)
                                                <img src="{{ asset('storage/' . $disfraz->imagen) }}" class="card-img-top" style="height: 200px; object-fit: cover;">
                                            @else
                                            ...
                                            <div class="card-body">
                                                ...
                                                <p class="text-muted"><strong>Precio:</strong> S/. {{ number_format($disfraz->precio, 2) }}</p>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
