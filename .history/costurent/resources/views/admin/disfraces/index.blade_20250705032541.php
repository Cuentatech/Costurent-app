...
<div class="container mt-5">
    ...
    @foreach($categorias as $categoria)
        @php
            ...
            if(request('categoria') && request('categoria') != $categoria->id) continue;
        @endphp
        ...
        @if($disfracesFiltrados->count())
            ...
            <div id="carouselCat{{ $categoria->id }}" class="carousel slide" data-bs-ride="carousel">
                <div class="carousel-inner">
                    @foreach($disfracesFiltrados->chunk(3) as $index => $chunk)
                                ...
                                @foreach($chunk as $d)
                                    <div class="col-md-4">
                                        <div class="card h-100 shadow-sm">
                                            @if($d->imagen)
                                                <img src="{{ asset('storage/' . $d->imagen) }}" class="card-img-top" alt="Imagen de {{ $d->nombre }}">
                                            @else
    ...
    <div class="table-responsive shadow-sm mt-5">
        <table class="table table-bordered table-hover align-middle">
            ...
            <tbody>
                @php $editarId = request('editar'); @endphp
                    ...
                    @if ($editarId == $d->id)
                        <form action="{{ route('admin.disfraces.update', $d->id) }}" method="POST">
                            ...
                            <tr>
                                ...
                                <td><input type="number" step="0.01" name="precio" value="{{ $d->precio }}" class="form-control" required></td>
                                <td>
...
<script>
    function mostrarFormulario(tipo) {
        const disfraz = document.getElementById('formulario-disfraz');
        const categoria = document.getElementById('formulario-categoria');

        if (tipo === 'disfraz') {
            disfraz.classList.toggle('d-none');
            categoria.classList.add('d-none');
        } else {
            categoria.classList.toggle('d-none');
            disfraz.classList.add('d-none');
        }
    }
</script>
@endsection
